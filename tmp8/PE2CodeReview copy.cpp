...
class Excercises {
        ...
        */
        ...
        for (int j=i+1; j<v.size(); j++) {
            if (v[i]->getShapeType()==v[j]->getShapeType()&&v[i]->getArea()==v[j]->getArea())
                return true;
            }
        }
            return false;
        ...
        static vector<Shape*> vectorUnion(vector<Shape*> v1, vector<Shape*> v2) {
            ...
            vector<Shape*> result(v1);//aqui ya pasaste un vecor
            //puedes reusar el codigo de duplicate
            bool found = false;
            for (int i=0; i<v2.size(); i++) {
                for (int j=i+1; j<result.size(); j++) {
                        if (v2[i]->getShapeType()==result[j]->getShapeType()&&v2[i]->getArea()==result[j]->getArea()){
                                found = true;
        ...
        static vector<Shape*> vectorIntersection(vector<Shape*> v1, vector<Shape*> v2) {
            vector<Shape*> v_inter;
            for(int i =0;i<v1.size();i++){
                for(int j=0;j<v2.size();j++){
                    if((v1[i]->getShapeType()==v2[j]->getShapeType()&&v1[i]->getArea()==v2[j]->getArea())){
                        v_inter.push_back(v1[i]);
