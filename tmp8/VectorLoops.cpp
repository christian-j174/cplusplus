...
void Account::removeAllNegative(vector<Account> &accounts)
{
   ...
   vector <Account>::iterator myIterator = accounts.begin();
   while(myIterator!=accounts.end()){
    if(myIterator->getBalance()<0){//*devuelve cuenta//de un iterator a cuenta de cuenta a balance//-> lo mismo
    myIterator=accounts.erase(myIterator);
    }
    else{
        myIterator++;
    }
